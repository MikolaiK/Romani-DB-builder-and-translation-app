...
export function DialectSelector() {
  ...
  return (
    <div className="flex items-center gap-2 text-xs">
      <label className="text-gray-600">Dialekt:</label>
      <div className="flex gap-1">
        {dOpts.map(d => (
          <button
            ...
            title={`Använd ${d} dialekt`}
          >{d}</button>
        ))}
      </div>
      <div className="w-px h-5 bg-gray-200 mx-1" />
      <label className="text-gray-600">Stil:</label>
      <select
        className="h-7 px-2 border rounded"
        value={style}
        onChange={(e) => setStyle(e.target.value as Style)}
      >
        {sOpts.map(s => <option key={s} value={s}>{s === 'Neutral' ? 'Neutral' : s === 'Formal' ? 'Formell' : 'Informell'}</option>)}
      </select>
    </div>
  );
}
